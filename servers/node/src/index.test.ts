...
describe("WebSocket Server", () => {
  ...
  describe("Room Management", () => {
    ...
    test("should return the same room instance", async () => {
      const roomId = "test-room-2";
      const room1 = await getOrCreateRoom(roomId);
      const room2 = await getOrCreateRoom(roomId);
  ...
  describe("Message Handling - join-room", () => {
    ...
    test("should handle multiple guests with duplicate names", async () => {
      ...
      const updatedRoom = await getOrCreateRoom(roomId);
      ...
      const finalRoom = await getOrCreateRoom(roomId);
