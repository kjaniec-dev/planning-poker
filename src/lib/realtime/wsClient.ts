...
function generateUUID(): string {
  ...
  // Fallback to crypto.getRandomValues (broader support)
  if (
    typeof crypto !== "undefined" &&
  ...
  // Final fallback to Math.random (least secure but works everywhere)
  return "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g, (c) => {
...
export function connectIfNeeded() {
  ...
  ws.onopen = () => {
    console.log("🔌 [wsClient] WebSocket connected to:", url);
    isConnecting = false;
    reconnectAttempts = 0;
  ...
  ws.onmessage = (event) => {
    try {
      ...
      if (msg.type === "room-state") {
        console.log("📥 [wsClient] Received room-state:", msg.data);
      }
      notifyListeners(msg);
    } catch (err) {
